...
async def scrape_retailer_products_meta(
    ...
    try:
        ...
        user_id = current_user.id
        logger.info(f"User authenticated with user_id: {user_id}")
        background_tasks.add_task(scrape_and_store_products_meta, retailer_id, user_id)
        return {
            "message": "Product scraping started",
            "retailer_id": retailer_id,
            "user_id": user_id,
...
async def scrape_and_store_products_meta(retailer_id: int, user_id: str):
    logger.info(f"Scraping products for retailer_id: {retailer_id}")
    try:
        page = 1
        all_products = {}
        ...
        while page <= total_pages:
            ...
            try:
                with httpx.Client() as client:
                    response = client.get(
                        "https://api.cannmenus.com/v1/products/meta",
                        params=params,
                        headers=headers,
                        timeout=10.0,
                    )
                ...
                for product_group in products:
                    meta_sku = product_group.get("meta_sku")
                    retailer_id = product_group.get("retailer_id")
                    for product in product_group.get("products", []):
                        cann_sku_id = product.get("cann_sku_id")
                        if cann_sku_id not in all_products:
                            all_products[cann_sku_id] = {
                                "retailer_id": retailer_id,
                                "meta_sku": meta_sku,
                                **product,
                            }
        ...
        updated_product_ids = set()
        count = 0
        for cann_sku_id, product in all_products.items():
            if not cann_sku_id:
                logger.warning(f"Skipping product without cann_sku_id: {product}")
                continue
            product_ref = products_ref.document(cann_sku_id)
            batch.set(
                product_ref,
                {
                    **product,
            ...
            updated_product_ids.add(cann_sku_id)
            count += 1
